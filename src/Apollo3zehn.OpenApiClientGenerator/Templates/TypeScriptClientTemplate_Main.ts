...
{{/Special_NexusFeatures}}
import { HttpRequestHandler, {{{ExceptionType}}} } from "./_shared";
{{{VersioningImports}}}

/**
 * A client for the {{{ClientName}}} system.
 */
export interface I{{{ClientName}}}Client {
 ...
 * A client for the {{{ClientName}}} system.
 */
export class {{{ClientName}}}Client implements I{{{ClientName}}}Client {
{{#Special_NexusFeatures}}
    private readonly _configurationHeaderKey = "{{{Special_ConfigurationHeaderKey}}}";
{{/Special_NexusFeatures}}
{{#Special_AccessTokenSupport}}
...
{{{VersioningFields}}}
    ...
    /**
     * Initializes a new instance of the {{{ClientName}}}Client.
...
{{/Special_NexusFeatures}}
        ...
        signal?: AbortSignal): Promise<T>
    {
        ...
        if (!response.ok) {
            ...
            const statusCode = `{{{ExceptionCodePrefix}}}00.${response.status}`;

            if (!message)
                throw new {{{ExceptionType}}}(statusCode, `The HTTP request failed with status code ${response.status}.`);
            else
                throw new {{{ExceptionType}}}(statusCode, `The HTTP request failed with status code ${response.status}. The response message is: ${message}`);
        }
        ...
        try {
            ...
            throw new {{{ExceptionType}}}("{{{ExceptionCodePrefix}}}01", "Response data could not be deserialized.");
        }
    }
